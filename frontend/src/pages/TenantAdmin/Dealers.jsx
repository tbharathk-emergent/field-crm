...
export default function Dealers() {
  ...
  const blank = {
    phone: "", name: "", role: "customer", business_name: "", dealer_code: "",
    address: "", village: "", district: "", state: "", pincode: "",
    gst_number: "", assigned_employee_id: "", credit_limit: 0, outstanding_amount: 0,
  };
  ...
  const label = getLabel(tenant, "customer_plural", "Dealers/Customers");
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          ...
          <button data-testid="add-dealer-btn" onClick={openCreate} className="btn-primary"><Plus size={16} /> Add</button>
        </div>
      </div>
      ...
      <div className="card-surface overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-brand-bg text-brand-mute text-xs uppercase tracking-wider">
            <tr>
              <th className="text-left px-4 py-3">Name / Business</th>
          ...
          <tbody className="divide-y divide-brand-line">
            {view.map((u) => (
              <tr key={u.id} className="hover:bg-brand-bg/50">
                <td className="px-4 py-3"><div className="font-medium">{u.name}</div><div className="text-xs text-brand-mute">{u.business_name}</div></td>
                <td className="px-4 py-3 font-mono text-xs">{u.phone}</td>
                <td className="px-4 py-3">{u.dealer_code || "—"}</td>
                <td className="px-4 py-3 text-brand-mute">{u.village || "—"}, {u.district || "—"}</td>
                <td className="px-4 py-3 text-right font-mono">₹{(u.outstanding_amount || 0).toLocaleString("en-IN")}</td>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-xl max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>{editing ? "Edit" : "Add"} {getLabel(tenant, "customer", "Customer")}</DialogTitle></DialogHeader>
          <div className="grid sm:grid-cols-2 gap-3 py-3">
            ...
            <div><Label>Code</Label><Input value={form.dealer_code || ""} onChange={(e) => setForm({ ...form, dealer_code: e.target.value })} /></div>
            ...
            <div><Label>Village/Town</Label><Input value={form.village || ""} onChange={(e) => setForm({ ...form, village: e.target.value })} /></div>
